...
# helper defs
def create_test_chat(fake_model="general_model", fake_context="my fake context", fake_topic="my fake topic", fake_question="I have a question.", fake_answer="I have an answer."):
    test_model = LLM_MODEL.create(model=fake_model, size=34567, specialization="general", currently_available=True)
    ...
    test_chat = Chat.create(question=fake_question, answer=fake_answer, context_id=test_context.id, topic_id=test_topic.id, llm_model_id=test_model.id)
    return test_chat
...
def test_check_file_type(test_database, model_name, f_specialization, file_name, tf):
    ...
    model_id = LLM_MODEL.create(model=model_name, size=34567, specialization=f_specialization, currently_available=True)
