...
from src.myllamatui.db_models import Category, Chat, Context, Topic, LLM_MODEL, CLI_Settings
...
def test_create_db(mock_create_tables, mock_connect):
    ...
    mock_create_tables.assert_called_once_with([Context, Category, Topic, Chat, LLM_MODEL, CLI_Settings], safe=True)  # Verify correct table list
...
async def test_initialize_db_defaults(test_database, mock_get):
    ...
    assert context_1.text == "You are a friendly and helpful assistant, always aiming to provide informative and comprehensive answers."
    assert context_2.text == "You are a friendly senior developer assisting a jounior developer by providing code examples and comprehensive explanations."
